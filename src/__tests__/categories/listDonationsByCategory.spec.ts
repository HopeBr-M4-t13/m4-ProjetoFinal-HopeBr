...
describe("GET - /categories/:id/donations", () => {
	...
	test("Should be able to list donations by category id", async () => {
		...
		const response = await request(app)
			.get(`/categories/${categoryTest.body.id}/donations`)
			.set("Authorization", `${adminData.token}`);
	...
	test("Should be able to list donations by category id | User no authentication", async () => {
		const categoryTest = await request(app)
			.post("/categories")
			.send(mockedCategory)
			.set("Authorization", `${adminData.token}`);
	...
	test("Should be able to list donations by category id | Id invalid", async () => {
        const response = await request(app)
        .get("/categories/04b591a2-76b3-4c95-bc4d-070cccac87b7/donations")
        .set("Authorization", `${adminData.token}`)
